...
class ReconScanner:
    ...
    async def run_subfinder(self) -> List[Dict[str, Any]]:
        ...
        if config.USE_DOCKER:
            cmd = ["docker", "run", "--rm", "-i", config.DOCKER_SUBFINDER_IMAGE, "-d", self.target_domain, "-json", "-silent"]
        else:
            cmd = ["subfinder", "-d", self.target_domain, "-json", "-silent"]
            
        logger.info(f"[recon] Running subfinder for {self.target_domain} (configured: {'docker' if config.USE_DOCKER else 'host'})")
    ...
    async def run_amass(self) -> List[Dict[str, Any]]:
        ...
        if config.USE_DOCKER:
            cmd = ["docker", "run", "--rm", "-i", config.DOCKER_AMASS_IMAGE, "enum", "-passive", "-d", self.target_domain, "-json", "stdout", "-silent"]
        else:
            cmd = ["amass", "enum", "-passive", "-d", self.target_domain, "-json", "stdout", "-silent"]
            
        logger.info(f"[recon] Running amass for {self.target_domain} (configured: {'docker' if config.USE_DOCKER else 'host'})")
    ...
    async def execute_discovery(self) -> List[ReconAsset]:
        """Run Subfinder only for subdomain discovery."""
        sub_results = await self.run_subfinder()
