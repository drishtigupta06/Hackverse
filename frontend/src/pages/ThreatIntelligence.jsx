...
import { AlertCircleIcon, TargetIcon } from "../components/Icons.jsx";
    ...
    return (
        <div className="fade-in">
            ...
            {loading ? (
                ...
                <div className="glass-card" style={{ padding: 40, color: "var(--red)", textAlign: "center", border: "1px solid var(--red)", background: "rgba(239, 68, 68, 0.05)" }}>
                    <div style={{ marginBottom: 15 }}><AlertCircleIcon size={30} style={{ color: "var(--red)" }} /></div>
                    ...
                    {feed.map((item, idx) => (
                        <div key={idx} className="glass-card transition-all" style={{ padding: 24, borderLeft: `4px solid ${item.severity === 'critical' ? 'var(--red)' : item.severity === 'high' ? 'var(--orange)' : 'var(--yellow)'}` }}>
                            ...
                            {(item.mitre_tactic || item.mitre_technique) && (
                                <div style={{ display: "inline-flex", gap: 12, alignItems: "center", background: "var(--bg-secondary)", padding: "10px 16px", borderRadius: 8, border: "1px solid var(--border)" }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ fontSize: 13, color: "var(--cyan)", fontWeight: 700, marginTop: 2 }}>
                                            {item.mitre_tactic} <span style={{ color: "var(--text-muted)", margin: "0 5px" }}>→</span> <span style={{ color: "var(--text-primary)" }}>{item.mitre_technique}</span>
                                        </div>
                                    </div>
                                </div>
                            )}
