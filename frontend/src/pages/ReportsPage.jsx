...
const TYPE_ICONS = {
    "Network Scan": TargetIcon,
    "LLM Scan": BrainIcon,
    "Mobile Scan": DocumentIcon,
};

const REPORT_SECTIONS = [
    { Icon: TargetIcon, label: "Target Info" },
    { Icon: SearchIcon, label: "Executive Summary" },
    { Icon: DashboardIcon, label: "Vulnerability Table" },
    { Icon: AlertCircleIcon, label: "Detailed Findings" },
    { Icon: GraphIcon, label: "Attack Graph Analysis" },
    { Icon: ShieldIcon, label: "Remediation Advice" },
    { Icon: HistoryIcon, label: "CVE References" },
    { Icon: ResultsIcon, label: "Security Recommendations" },
];
    ...
    useEffect(() => {
        Promise.all([
            ...
            const doneNmap = nmapScans.filter(s => s.status === "completed").map(s => ({
                ...
                vulns: s.vuln_count || 0, downloadFn: api.downloadReport
            }));
            ...
            const doneLLM = llmJobs.filter(s => s.status === "completed").map(s => {
                ...
                return {
                    ...
                    vulns: fails, downloadFn: api.downloadLLMReport
                };
            });
            ...
            const doneMobile = mobileScans.filter(s => s.status === "completed").map(s => {
                ...
                return {
                    ...
                    vulns: fails, downloadFn: api.downloadMobileReport
                };
            });
        ...
        <div className="fade-in">
            <div className="page-header">
                <div>
                    <div className="page-title" style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
                        <DocumentIcon size={24} style={{ color: "var(--accent)" }} /> Reports
                    </div>
            ...
            {scans.length === 0 ? (
                <div className="empty-state">
                    <DocumentIcon size={48} style={{ opacity: 0.4, marginBottom: 12 }} />
                ...
                <div style={{ display: "grid", gap: 16 }}>
                    {scans.map(scan => {
                        const TypeIcon = TYPE_ICONS[scan.type] || DocumentIcon;
                        return (
            ...
            {scans.length > 0 && (
                <div className="card" style={{ marginTop: 24 }}>
                    <div className="card-title" style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                        <ResultsIcon size={18} style={{ color: "var(--accent)" }} /> Report Contents
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
                        {REPORT_SECTIONS.map(({ Icon, label }) => (
                            <div key={label} style={{
                                display: "flex", alignItems: "center", gap: 8, padding: "8px 12px",
                                background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)"
                            }}>
                                <Icon size={14} style={{ color: "var(--accent-light)", flexShrink: 0 }} />
