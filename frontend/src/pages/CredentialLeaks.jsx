    ...
    return (
        <div className="fade-in">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 30 }}>
                ...
                <button onClick={fetchJobs} className="btn btn-outline" style={{ fontSize: 12, display: "inline-flex", alignItems: "center", gap: 6 }}><RefreshCw size={14} /> Refresh Status</button>
            </div>

            <div className="glass-card" style={{ padding: 24, marginBottom: 30, background: "rgba(99,102,241,0.04)", border: "1px solid rgba(99,102,241,0.15)" }}>
                <form onSubmit={handleScan} style={{ display: "flex", gap: 12 }}>
                    ...
                    <button type="submit" className="btn btn-primary" disabled={loading} style={{ minWidth: 160, background: "var(--accent)" }}>
                        ...
                        {jobs.map((job) => (
                            <div
                                ...
                                style={{
                                    padding: 12,
                                    cursor: "pointer",
                                    background: selectedJob?.scan_id === job.scan_id ? "rgba(99,102,241,0.08)" : "rgba(255,255,255,0.03)",
                                    border: selectedJob?.scan_id === job.scan_id ? "1px solid var(--accent-light)" : "1px solid var(--border)",
                ...
                <div className="glass-card" style={{ padding: 24 }}>
                    ...
                    {!selectedJob && (
                        <div style={{ textAlign: "center", padding: 60, color: "var(--text-muted)" }}>
                            <Search size={40} style={{ opacity: 0.5, marginBottom: 15 }} />
                    ...
                    {selectedJob && !loading && findings.length === 0 && (
                        <div style={{ textAlign: "center", padding: 40, color: "var(--green)" }}>
                            <Shield size={40} style={{ marginBottom: 15, color: "var(--green)" }} />
