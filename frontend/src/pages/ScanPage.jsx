...
export default function ScanPage({ user }) {
    ...
    async function handleScan(e) {
        ...
        addLog(`Scan engines: ${types.join(", ")}`);
            ...
            <div className="page-header">
                <div>
                    <div className="page-title" style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
                        <TargetIcon size={24} style={{ color: "var(--accent)" }} /> Launch Scan
                    </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                ...
                <div className="card">
                    <div className="card-title" style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                        <SettingsIcon size={18} style={{ color: "var(--accent)" }} /> Scan Configuration
                    </div>
                    <form onSubmit={handleScan} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                        <div className="input-group">
                            ...
                            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                                Tip: Use <code style={{ color: "var(--cyan)" }}>127.0.0.1</code> for a quick demo (runs instantly without Celery)
                            </div>
                        </div>
                        ...
                        <div className="input-group">
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                                {[
                                    { key: "nmap", label: "Nmap", desc: "Port scan + service detection + vulnerability scripts" },
                                    { key: "acunetix", label: "Acunetix DAST", desc: "Web app vulnerability scan (requires API key)" },
                                ].map(({ key, label, desc }) => (
                                    <label key={key} style={{
                                        display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", cursor: "pointer",
                                        background: scanTypes[key] ? "rgba(79,70,229,0.05)" : "var(--bg-card)",
                        ...
                        <button className="btn btn-primary btn-lg" type="submit" disabled={loading}
                            ...
                            {loading
                                ? <><div className="spinner" style={{ width: 18, height: 18, marginRight: 8 }} /> Scanning...</>
                                : "Launch Scan"}
                        </button>
                    </form>
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    {result && (
                        <div className="card">
                            <div className="card-title" style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                                <TargetIcon size={18} style={{ color: "var(--accent)" }} /> Scan Status
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                                {[
                                    ...
                                    ["Findings", `${result.vuln_count || 0} findings`, null],
                                ].map(([k, v, color]) => (
                                    <div key={k} style={{
                            ...
                            {result.status === "completed" && (
                                <div style={{ display: "flex", gap: 10 }}>
                                    ...
                                    <button className="btn btn-secondary" onClick={() => navigate(`/graph/${result.scan_id}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    ...
                    <div className="card" style={{ flex: 1 }}>
                        <div className="card-title" style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                            <ResultsIcon size={18} style={{ color: "var(--accent)" }} /> Live Scan Log
                        ...
                        <div style={{
                            ...
                            {logs.length === 0 ? (
                                ...
                                <div key={i} style={{
                                    color: l.type === "error" ? "var(--red)" : l.type === "success" ? "var(--green)" : "var(--text-secondary)",
