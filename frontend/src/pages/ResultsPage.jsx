...
import { SearchIcon, GraphIcon, BrainIcon, ChatIcon, ResultsIcon } from "../components/Icons.jsx";
    ...
    return (
        <div className="fade-in">
            <div className="page-header">
                <div>
                    <div className="page-title" style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
                        <SearchIcon size={24} style={{ color: "var(--accent)" }} /> Scan Results
                    </div>
                    <div className="page-subtitle">{selectedScan ? `Viewing scan: ${selectedScan?.slice(0, 8)}...` : "Select a completed scan"}</div>
                </div>
                {selectedScan && (
                    <div style={{ display: "flex", gap: 12 }}>
                        <button className="btn btn-secondary" onClick={() => navigate(`/graph/${selectedScan}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><GraphIcon size={14} /> Attack Graph</button>
                        <button className="btn btn-secondary" onClick={() => navigate(`/simulation/${selectedScan}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><BrainIcon size={14} /> AI Simulation</button>
                        <button className="btn btn-primary" onClick={() => navigate(`/chat/${selectedScan}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><ChatIcon size={14} /> Ask AI</button>
                    </div>
                )}
            </div>
            ...
            <div className="card" style={{ marginBottom: 24 }}>
                <div className="card-title" style={{ marginBottom: 12, display: "inline-flex", alignItems: "center", gap: 8 }}>
                    <ResultsIcon size={18} style={{ color: "var(--accent)" }} /> Select Scan
                </div>
                ...
                <>
                    ...
                    <div style={{ display: "flex", gap: 4, marginBottom: 16, borderBottom: "1px solid var(--border)", paddingBottom: 0 }}>
                        {[
                            ...
                            { id: "hosts", label: `Hosts & Ports (${hosts.length})` },
                        ].map(t => (
                    ...
                    {loading ? (
                        ...
                        sorted.length === 0 ? (
                            <div className="empty-state">
                                ...
                                <div className="empty-state-desc">No issues matching your filters</div>
                            </div>
                        ) : (
                        ...
                        hosts.length === 0 ? (
                            <div className="empty-state">
                                <GraphIcon size={48} style={{ opacity: 0.5, marginBottom: 12 }} />
                            ...
                            <div key={hi} className="card" style={{ marginBottom: 16 }}>
                                <div className="card-title" style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                                    <GraphIcon size={16} style={{ color: "var(--accent)" }} /> {h.host}
                                </div>
                                <div className="table-container">
                                    <table>
